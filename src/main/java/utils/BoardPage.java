...
public class BoardPage {
	...
	public static String pagingStr(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
		...
		현재 페이지 블럭에서 사용할 첫번째 페이지번호를 계산한다.
			...
			현재 5페이지라 가정하면
				(((5 - 1) / 5) * 5) + 1 = 1
					==> 즉 1~5 페이지인 경우 해당블럭의 첫번째 페이지는 1이다.
			현재 6페이지라 가정하면
				(((6 - 1) / 5) * 5) + 1 = 6
			현재 11페이지라 가정하면
				(((11 - 1) / 5) * 5) + 1 = 11 	
		*/
		...
		/*
		pageTemp가 1이 아닐 때만, 즉 첫번째 블럭이 아닐 때만 이전블럭 바로가기 링크를 화면에 출력한다.
		*/
		if(pageTemp != 1) {
		...
		이 때 1씩 증가시켜준다. 즉 한블록당 5페이지를 출력하게 된다.
		*/
		int blockCount = 1;
		while(blockCount <= blockPage && pageTemp <= totalPages) {
			//만약 현재페이지라면 링크를 걸지 않는다. a태그 제거
			if(pageTemp == pageNum) {
				pagingStr += "&nbsp;" + pageTemp + "&nbsp;";
			}
			else {
