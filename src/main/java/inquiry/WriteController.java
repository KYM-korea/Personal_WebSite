...
public class WriteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (mode.equals("insert")) {
			...
			dao.close();
			JSFunction.alertLocation(resp, "삭제가 완료되었습니다.", "../inquiry/inquiryList.do");
		}
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		ServletContext application = getServletContext(); 
			int maxPostSize = Integer.parseInt(application.getInitParameter("POSTMAXSIZE"));
		  
		MultipartRequest mr = FileUtil.uploadFile(req, saveDirectiory, maxPostSize);
	
		if(mr == null) { 
			JSFunction.alertLocation(resp, "첨부 파일이 제한 용량을 초과합니다.", "../inquiry/inquiryWrite.do");
			return; 
		} 
		...
		if (mode.equals("insert")) {
			...
			result = dao.updateInquiry(dto);
		}
		dao.close();
		 
		//서블릿에서 특정 요청명으로 이동할 때는 sendRedirect()를 사용 
		if (result == 1) {
			resp.sendRedirect("../inquiry/inquiryList.do"); 
		}
		else {
